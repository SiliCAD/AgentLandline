"""
Test suite for AgyPipeline.
Verifies multi-turn memory, tool execution, and question handling.
"""
...
def test_multi_turn_and_tools():
    ...
    pipeline.start()
    print(f"Agent Conversation ID: {pipeline.conversation_id}")
    ...
    try:
        ...
        print("\n----------------------------------------")
        print("Turn 3: Testing tool execution ('Read requirements.txt with view_file')...")
        t3 = time.time()
        res3 = pipeline.send("Use the view_file tool to view requirements.txt in the current directory.")
