...
class IssueReporter:
    ...
    def find_matching_label(cls, target: str, existing_labels: List[str]) -> Optional[str]:
        """
        Performs smart check for label in existing GitHub labels:
        1. Exact match (case-sensitive)
        2. Case-insensitive match
        3. Separator-normalized match (e.g. 'new_lable' <-> 'new lable' <-> 'new-lable')
        4. Alphanumeric match (e.g. ignoring all punctuation differences)
        5. Fuzzy similarity match for minor typos (similarity ratio >= 0.82)
        """
        ...
        # 3. Separator/delimiter normalized match
        for lbl in existing_labels:
    ...
    def resolve_or_create_label(
        ...
        """
        Resolves a label name against existing labels or creates a new one with a random color.
        
        Returns:
            The canonical label name on GitHub, or None if creation failed.
        """
        ...
        # Label does not exist -> create new label with random color
        ...
        try:
            ...
            logger.warning(f"Label creation returned error for {clean_name!r}: {e.stderr.strip()}")
    ...
    def ensure_label_exists(cls, label_name: str, cwd: Optional[str] = None) -> bool:
        """Checks if a label exists in the GitHub repository, creating it if missing."""
        existing_labels = cls.get_existing_labels(cwd=cwd)
        resolved = cls.resolve_or_create_label(label_name, existing_labels, cwd=cwd)
    ...
    def create_issue(
        ...
        # Load repository labels once for smart matching
        existing_labels = cls.get_existing_labels(cwd=cwd)

        labels_to_apply: List[str] = []

        if label and label.strip():
            # Support comma-separated labels if provided
            ...
            for lbl in raw_labels:
                resolved = cls.resolve_or_create_label(
                    ...
                    existing_labels,
                    cwd=cwd,
                    default_description=f"Auto-created issue label: {lbl}"
                )
